...
void movimento_inimigos(tipo_jogador inimigos[], tipo_mapa *mapa, tipo_jogador *jogador, int *endgame);
int diferenca_tempo(clock_t start, float set);
void HideCursor();
...
void movimento_inimigos(tipo_jogador inimigos[], tipo_mapa *mapa, tipo_jogador *jogador, int *endgame){
    ...
    if(diferenca_tempo(start, VELOCIDADE)==1){
        k=0;
        for(i=0; i<MAXEN; i++){
            x = inimigos[i].x-1;
            y = inimigos[i].y-2;
